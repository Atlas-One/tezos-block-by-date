...
module.exports = class {
    constructor(baseURL, chain = 'main') {
    ...
    async getDate(date, after = true) {
        ...
        if (date.isSameOrAfter(moment.unix(this.latestBlock.timestamp)))
            return this.returnWrapper(date.format(), this.latestBlock.block);
    ...
    async findBetter(date, predictedBlock, after, blockTime = this.blockTime) {
        if (await this.isBetterBlock(date, predictedBlock, after)) return predictedBlock.block;
        ...
        let nextPredictedBlock = await this.getBlockWrapper(
            this.getNextBlock(date, predictedBlock.block, skip)
        );
        blockTime = Math.abs(
            (parseInt(predictedBlock.timestamp, 10) - parseInt(nextPredictedBlock.timestamp, 10)) /
                (parseInt(predictedBlock.block, 10) - parseInt(nextPredictedBlock.block, 10))
        );
    ...
    async isBetterBlock(date, predictedBlock, after) {
        ...
        if (after) {
            ...
            let previousBlock = await this.getBlockWrapper(predictedBlock.block - 1);
            if (
            ...
            let nextBlock = await this.getBlockWrapper(predictedBlock.block + 1);
    ...
    async getBlockWrapper(block) {
        block =
            block == 'head'
                ? (await this.client.get(`/chains/${this.chain}/blocks/head`)).data.header.level
                : block;
        ...
        } = await this.client.get(`/chains/${this.chain}/blocks/${block}`);
